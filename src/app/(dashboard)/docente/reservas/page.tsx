...
import { getMisReservas, cancelarGrupoReservas } from "@/lib/services/reservaService";
        ...
        const map = data.data.reduce((acc: any, r: any) => {
            if (!acc[r.grupo_solicitud_id]) {
                const fechaSolicitud = r.created_at
                    ? new Intl.DateTimeFormat('es-ES', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(r.created_at))
                    : 'N/A';
    ...
    return (
        <div className="p-4 md:p-8 w-full max-w-[95%] xl:max-w-[1800px] mx-auto flex flex-col h-full space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 shrink-0">
                ...
                <Link href="/docente/reserva/nueva" className="bg-[#001D4A] hover:bg-[#001D4A]/90 text-white px-6 py-3 rounded-xl flex items-center gap-2 transition-all font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 whitespace-nowrap">
            ...
            <div className="flex-1 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col overflow-hidden">
                ...
                <div className="bg-white px-8 py-5 border-t border-slate-200 flex justify-between items-center shrink-0">
                    <p className="text-sm font-medium text-slate-500">
                        Visualizando página <span className="font-bold text-[#001D4A] bg-[#001D4A]/5 px-2.5 py-1 rounded-lg ml-1">{page}</span>
                    </p>
