...
interface Props {
    ...
    onToggleMulti: (celdas: CeldaSeleccionada[], action: 'add' | 'remove') => void;
}
...
export function ShiftGridSelector({ bloques, fechasVisibles, disponibilidad, seleccion, onToggleCelda, onToggleMulti }: Props) {
    ...
    const { libresDelTurno, isAllSelected } = useMemo(() => {
        ...
        fechasVisibles.forEach(fechaObj => {
            ...
            bloquesDelTurno.forEach(bloque => {
                const ocupado = disponibilidad.some(d => d.fecha === fechaStr && d.bloque_horario_id === bloque.id);
                if (!ocupado) {
                    libres.push({ fecha: fechaStr, bloqueId: bloque.id });
                    const isSelected = seleccion.some(s => s.fecha === fechaStr && s.bloqueId === bloque.id);
                    if (isSelected) seleccionadosEnTurno++;
                }
            });
        });
    ...
    const getEstadoCelda = (fechaStr: string, bloqueId: number) => {
        const isSelected = seleccion.some(s => s.fecha === fechaStr && s.bloqueId === bloqueId);
        if (isSelected) return { label: 'Seleccionado', clase: 'bg-blue-600 border-blue-700 text-white shadow-md shadow-blue-500/20 scale-[1.02] ring-2 ring-blue-600 ring-offset-1' };

        const reserva = disponibilidad.find(d => d.fecha === fechaStr && d.bloque_horario_id === bloqueId);
        if (!reserva) return { label: 'Libre', clase: 'bg-emerald-50/50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300 transition-colors' };

        if (reserva.estado === 'aprobada') return { label: 'Ocupado', clase: 'bg-slate-50 border-slate-200 text-slate-400 opacity-60 cursor-not-allowed' };
        if (reserva.estado === 'pendiente') return { label: 'Pendiente', clase: 'bg-amber-50 border-amber-200 text-amber-600 opacity-60 cursor-not-allowed' };
